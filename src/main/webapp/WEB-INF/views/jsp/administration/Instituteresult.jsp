...
<%@ page import="java.io.*"%>
<c:url var="commonreport" value="/taskforce/service/result/admin/report" />
		...
		<div class="content-wrapper">
			<section class="content">
				<div class="box-body" style="margin-top: 20px">
					<span class="pull-left"><font size="5"><b>All Institute Department Result:</b></font></span>
				</div>
								...
								<table id="example1" class="table table-bordered table-striped">
									<thead>
										<tr class="success">
											...
											<th style="width: 270px;">Department Name</th>	
										    <th>Exam Year</th>	
										    <th>Academic year</th>									
									...
									<tbody>
																<c:forEach var="list" items="${resultist}">
				                       <c:set var="n" value="${n + 1}" />
												...
												<c:forEach var="dept" items="${departmets}">
																<c:if test="${dept.dep_id==list.branch}">
																	<td>${dept.dep_name}</td>
																</c:if>
															</c:forEach>
												
																		<td>${list.year}</td>
																		<td>${list.academiyr}</td>
	<td>
						<form action="${commonreport}" method="post">																
	                       <input type="hidden" name="yearId" value="${list.year}">				          
                          <input type="hidden" name="branchId" value="${list.branch}">
                          <input type="hidden" name="classId" value="${list.academiyr}"> 
